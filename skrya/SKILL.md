...
- Before the first topic-scoped action, check the resolved data-root source. If `skrya data-root` reports `default-home`, and the user has not already supplied a data location in the current request, treat Skrya as not initialized.
- When Skrya is not initialized, pause the original workflow. The initialization question must be the sole user-facing response: no greeting, status text, rationale, capability summary, topic clarification, source plan, or next-step preview before or after it.
- In Chinese, output exactly this question interaction; translate it faithfully when the user's current language is English:
...
- After the user answers, persist the selection with the data-root flow, then resume the original request. Do not ask this initialization question when an explicit data root, `SKRYA_DATA_ROOT`, a workspace config, or a home config already resolves the data location.
...
- After any uninstall action, report a fixed four-category summary: `skill`, `data-root`, `data-config`, and `global-instruction`. For each category say `removed`, `kept`, `not found`, or `skipped`, and include the mode that was executed. Do not replace this with a vague "done".
...
- Route requests to create or update a topic-specific digest template through `topic-curation`. Before editing, tell the user that the active-Thread timeline section is mandatory and that the candidate will be checked before save. Validate the complete candidate before overwriting the configured template; a failed validation must leave the existing template untouched.
...
- before calling an automation tool or telling the user to forward a prompt, check the automation contract against the required field list below; if a field is unknown, write `unknown` or the concrete fallback rule instead of omitting it
...
- The first visible line of the response must be the digest title. Do not put status text, commentary, saved-file notes, or implementation notes before the title.
...
When creating or proposing a recurring digest automation, use this checklist as the contract. The prompt must be able to run in a fresh scheduled agent session without relying on the current chat.

- workflow: use Skrya's `digest` workflow for a resolved topic; do not ask for a generic news summary
- topic identity: include `topic-id` and visible topic name
...
- failure behavior: if a required file, source set, template, or delivery binding is missing, report the missing precondition rather than generating an unrelated summary
