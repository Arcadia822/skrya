...
1. 先解析 data root、topic 和当前已配置模板。
2. 在修改前明确提示：布局和措辞可以调整，但 active Thread 的“事件线时间线更新”模块属于强制合同；无新增时仍须展示最新状态。
3. 在不覆盖现有模板的情况下生成完整候选版本。
4. 保存前检查标题、事件线模块、Thread 名称和 ID、今日增量、时间线位置、影响判断、无新增复核、最新状态、下一步观察、编号简讯、逐条信源、`---` 和系统提示。
5. CLI 可用时，对候选文件运行 `skrya validate-digest-template --file <candidate-path>`。
6. 检查失败时保留原模板，列出缺失项并修订候选；检查通过后才保存，并说明保存前检查已通过。
7. 保存完成后单独询问是否试跑，不要把正式 artifact 写入模板更新流程。
