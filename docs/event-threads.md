...
`event-thread` 是一个比 `topic` 更小、比单条 digest item 更稳定的运行时实体，中文建议叫**事件线**。
...
这里推荐英文名 `event-thread`，中文名 **事件线**。

原因：

- `thread` 天然带有“串联、续写、回看上下文”的含义。
- “事件线”比“子 topic”更贴近用户语言，用户会自然说“继续跟这条线”。
  ...
  "threads": [
    {
      ...
      "name": "比亚迪闪充站",
      "status": "active",
      "summary": "围绕比亚迪闪充站建设、城市落地、合作扩张和实测反馈持续发展的事件线。",
      "callback_hint": "以后出现新站点开通、覆盖城市扩张、合作方接入或真实补能体验数据时，优先续写到这条事件线。",
...
| --- | --- | --- |
| `topic` | 用户长期关注面 | 最大 |
| `event-thread` | 某条持续发展的事件线 | 中间层 |
| digest item | 某次简报里的单条发现 | 最细 |

建议关系是：

1. ingest / digest 先发现当天的重要简讯。
2. 如果它属于某条已存在的持续事件，就把它挂到对应 `event-thread`。
3. 当天 digest 里可以继续输出这条简讯，但表述上应该让用户感知到它是“这条线的新进展”。
4. deep-analysis 可以既支持“第 3 条”，也支持“展开比亚迪闪充站这条线”。

事件线更新不要压成一行泛泛而谈。用户已经明确要求持续追踪时，日报应给 2-4 行具体内容：

```markdown
## 事件线更新

**比亚迪闪充站**
今天命中的简讯：1、2。首批落地城市名单开始清晰；媒体开始出现实测补能效率报道。
具体进展：这条线从发布会概念走到建设兑现，讨论重点已经变成站点是否真的铺开，以及宣传口径能否被真实补能体验支撑。
...
以 `topic = 新能源汽车`、`event-thread = 比亚迪闪充站` 为例：

1. 用户先建立长期 topic：关注新能源汽车的重要动态。
2. agent 在多天简报里反复看到“比亚迪闪充站发布、首批城市、合作建设、体验反馈”等相关进展。
3. 当这类简讯开始连续出现时，agent 应把它们识别为同一条持续事件线，而不是每天重写一个新标题。
4. 之后的日报应该用 2-4 行具体说明，而不是只写“这条线今天又有推进”。例如：今天命中的简讯编号、首批城市名单的变化、实测补能效率是否出现更多独立来源、后续还要盯什么。
...
仓库里附了一个结构化示例：[`byd-flash-charge-event-thread.example.json`](./byd-flash-charge-event-thread.example.json)。
