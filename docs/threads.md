...
- 可继续操作：
  ...
  - track: 创建新的thread，例如：`track: 3 4 5 持续关注`。
  - 偏好反馈：直接用自然语言回复，例如：`6 7 我不喜欢，如果是 xxx 不要关注`。
```

active thread 不能因为当天没有新增而从日报里消失。无新增时仍然展示最近状态：

```markdown
## 事件线时间线更新

┌─ **【Thread】比亚迪闪充站 (byd-flash-charge-station)**
│ 复核结果：已检查，本轮暂无可核验新增。
│ 最新状态：2026-04-27：首批站点城市名单开始清晰，媒体出现实测补能效率报道。
│
│ 下一步观察：首批站点在哪些城市真正落地；真实补能效率是否被更多独立来源验证。
└
```
