---
name: topic-curation
description: Use when the user wants to create or refine a tracked topic through natural language, or asks for important information, ongoing tracking, or a briefing about an area that is not yet a configured topic.
---
# Topic Curation
...
Also inherit applicable workspace defaults from `AGENTS.md`.

## Core Rule

先把用户真正想持续看到的简报内容聊清楚，再把用户的话抽象成稳定的配置语义。向用户确认拟写入的配置文字后，才创建或修改 topic 文件。
...
Source feasibility is simplified:
...
- “我想跟踪 AI 编程工具” -> new topic creation
- “增加 xx 新闻” -> add a new tracked angle
- “我不想关注第 5 条这种内容” -> exclude a kind of item from future digests
- “多看产品发布，少看融资” -> adjust ranking preference
- “帮我整理某个公司和其所在市场的重要资讯” -> first clarify the tracking request, then prepare topic configuration
- “我想持续跟踪某个行业，但还没想好具体抓什么” -> first clarify the tracking request, then prepare topic configuration
...
9. In user-facing conversation, it is fine to call them “信源” instead of configuration jargon.
10. When recommending sources, use the simple rule that RSS means connectable and no RSS means not connectable for now.
11. Confirm source additions or removals before writing `sources.json`.
...
- accept user language as-is
- infer the underlying lasting preference
- rewrite it into config-ready wording
- ask for confirmation
- then write files

A good confirmation format is:

“我理解你的意思是：以后重点看 X，弱化 Y。我准备写成下面这条配置……”
...
When the conversation reaches sources or the user asks to补信源:

- recommend sources based on the confirmed topic intent
- prefer sources that repeatedly surface the desired event types
- reject sources that have no RSS for now
- explain clearly when a desired source is not currently connectable

## Success Criteria

After this skill completes:

- the user feels understood in their own language
- the topic config reflects stable preferences, not one-off phrasing
- new topics are only created after intent is clear
- source choices follow confirmed topic intent and the RSS-only rule
