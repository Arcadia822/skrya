...
10. End the digest body, insert `---`, and write a `## 系统提示` section with execution metadata, `dig:` / `track:` options, and a plain-language preference feedback option.
11. For a test run, still use the same digest template and `## 系统提示` format, mark execution status as test/preview, explain `dig: <编号>`, `track: <编号> <名称/意图>`, and plain-language preference feedback, and do not mention saved artifacts.
